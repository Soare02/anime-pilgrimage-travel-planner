...
你是一个动漫圣地巡礼验证专家。
请仔细评估以下检索到的网页搜索结果，判断其是否能为地标 "{name}" 在动漫《{bangumi}》（集数: {ep}, 时间戳: {timestamp}）的圣地巡礼规划提供真实可用的信息。

判定标准请兼顾召回率和准确性：
- 如果结果同时提到该作品/系列与该地标，或明确是该作品巡礼攻略、舞台探访、取景地列表、打卡游记，则应判定为 relevant=true。
- 如果结果没有精确到集数或时间戳，但提供了同作品同地标的交通、地址、拍照机位、访问礼仪、游记经验、周边路线，也应判定为 relevant=true。
- 如果结果只提供该地标的通用旅游信息，但可用于当前巡礼路线安排（交通、开放时间、拍照位置、注意事项），且没有和作品/地点冲突，可判定为 relevant=true，并在 reason 中说明是“通用但可用于巡礼规划”的信息。
- 只有在结果明显是广告、技术页面、同名异地、其他作品、无法用于路线/场景/交通/拍照规划时，才判定为 relevant=false。

网页搜索结果内容：
{search_result}

请输出一个 JSON 对象，包含以下字段：
1. "relevant": 布尔值（true 或 false），如果搜索结果包含同作品同地标的巡礼资料，或包含可直接用于该地标巡礼规划的交通/地址/拍照/礼仪/路线信息，则为 true；否则为 false。
2. "reason": 字符串，简短说明判定相关或不相关的理由。
3. "extracted_details": 字符串，如果相关，请从搜索结果中提取并整理出关于该地标的圣地巡礼有用信息（如来源URL、具体地址、最近车站交通、名场面背景、最佳打卡时段、拍照角度、注意事项等）。如果判定为不相关，则该字段必须为 null。
